...
describe('/platform-users', () => {
  ...
  function assertLog (user, action, platformUser, isWritten) {
    if (!isWritten) {
      ...
      assert.isTrue(
        lastLine.includes(
          `admin:${user} ${action} platformUser:${platformUser}`
        )
      );
    }
  }
    ...
    describe('when user deletion is disabled', () => {
      ...
      before(() => {
        const app2 = new Application({
          platformSettings: {
            vars: {
              API_SETTINGS: { settings: { ACCOUNT_DELETION: { value: [] } } }
            }
          }
        });
